...
- Obsługiwany jest pojedynczy SELECT z jednym `FROM {{source}}`, bez JOIN, UNION, CTE i podzapytań odczytujących inne tabele. Źródło musi być tabelą Delta, nie widokiem. Każda reguła czyta zapisaną wersję Delta; różne reguły mogą odczytać różne wersje, jeśli źródło zmieni się w trakcie zadania.
