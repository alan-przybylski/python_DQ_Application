...
def test_demo_contains_synthetic_history_and_never_resets_it(tmp_path):
    ...
    try:
        ...
        assert (
            connection.execute("SELECT COUNT(*) FROM data_load_log").fetchone()[0] == 3
        )
        assert (
            connection.execute(
                "SELECT COUNT(*) FROM customers WHERE email NOT LIKE '%@example.test'"
            ).fetchone()[0]
            == 0
        )
        assert (
            connection.execute(
                "SELECT SUM(failed_count) FROM dq_results WHERE timestamp LIKE '2026-09-03%'"
            ).fetchone()[0]
            == 0
        )
