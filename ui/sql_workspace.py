...
class SqlWorkspace:
    def __init__(self, root, username, role, dashboard_root, time_var):
        ...
        sidebar.rowconfigure(3, weight=1, minsize=45)
        tk.Label(sidebar, text=tr("Dataset tables"), font=("Segoe UI", 11, "bold")).grid(row=0,column=0,sticky='w')
        tk.Button(sidebar, text=tr("Refresh"), command=self.refresh).grid(row=1,column=0,sticky='ew',pady=3)
        tk.Button(sidebar,text=tr('Import tables'),command=self.open_import).grid(row=2,column=0,sticky='ew',pady=3)
        ...
        tk.Label(sidebar, text=tr("Double-click to insert a table or column."),
                 wraplength=210, justify="left", foreground=MUTED).grid(row=4,column=0,sticky='w',pady=6)
        tk.Button(sidebar, text=tr("Preview table"), command=self.table_query).grid(row=5,column=0,sticky='ew',pady=3)
        tk.Button(sidebar, text=tr("Rule template"), command=lambda: self.table_query(rule=True)).grid(row=6,column=0,sticky='ew',pady=3)
        tk.Button(sidebar,text=tr('Send table to Databricks'),command=self.open_cloud).grid(row=7,column=0,sticky='ew',pady=3)
        ...
        editor_frame.grid(row=0,column=0,sticky='nsew',pady=6)
        ...
        toolbar.grid(row=1,column=0,sticky='ew',pady=6)
        ...
        tk.Label(top, text=tr("Runs selected SQL, or the whole editor. Read-only · up to 500 rows · 10 seconds."),
                 anchor="w", wraplength=760, foreground=MUTED).grid(row=2,column=0,sticky='ew',pady=(0,8))
