...
class CloudWindow:
    def __init__(self,root,username,role,parent,time_var,initial_table=None,initial_rule=None):
        ...
        header(root,'Data transfers / synchronization',username)
        ...
        tk.Label(body,text=tr('Transfers create separate copies. To query cloud tables directly, choose the Databricks workspace.'),wraplength=950).pack(anchor='w',pady=8)
        ...
        button(table_actions,'Create local snapshot from Databricks',self.download_table)
        ttk.Separator(body).pack(fill='x',pady=10)
        tk.Label(body,text=tr('Rule')).pack(anchor='w')
        c=get_connection()
        try: rules=c.execute("SELECT id,description,target_table FROM dq_rules WHERE sql_engine='sqlite' ORDER BY id").fetchall()
        finally: c.close()
