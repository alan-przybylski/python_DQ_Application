...
def create_demo_database(destination=None):
    ...
    if destination == DATABASE_PATH.resolve():
        raise ValueError(
            "Demo data must never be written to the private application database."
        )
        ...
        try:
            if (
                ...
                raise FileExistsError(
                    "Existing file is not a DQ demo database; left unchanged."
                )
        finally:
            connection.close()
    ...
    try:
        with connection:
            connection.execute(
                ...
                (
                    DEMO_USERNAME,
                    bcrypt.hashpw(DEMO_PASSWORD.encode(), bcrypt.gensalt()).decode(
                        "ascii"
                    ),
                    "superuser",
                ),
            ...
            for rule_id, description, rule_type, error, sql in RULES:
                connection.execute(
                    """INSERT INTO dq_rules(id,version,description,rule_type,target_table,error_message,sql_query)
            ...
            partial = [
                clean_row if dirty_row[0] <= 6 else dirty_row
                for dirty_row, clean_row in zip(dirty, clean)
            ]
            batches = (
                (dirty, "2026-09-01 09:00:00", "customers_with_issues.csv"),
            ...
            for rows, timestamp, filename in batches:
                run_id = connection.execute(
                    """INSERT INTO dq_runs(table_name,username,started_at,completed_at,mode,status,rules_requested,rules_completed)
                    VALUES('customers',?,?,?,'all','completed',3,3)""",
                    (DEMO_USERNAME, timestamp, timestamp),
                ).lastrowid
                ...
                connection.execute(
                    """INSERT INTO data_load_log(table_name,file_name,row_count,loaded_by,loaded_at)
                    VALUES('customers',?,?,?,?)""",
                    (filename, len(rows), DEMO_USERNAME, timestamp),
                )
                for rule_id, _, _, error, sql in RULES:
                    ...
                    connection.execute(
                        ...
                        VALUES(?,'1.0',?,?,?,?)""",
                        (rule_id, len(records) - passed, passed, timestamp, run_id),
                    )
                    for record in records:
                        field = list(record)[1]
                        connection.execute(
                            """INSERT INTO dq_field_results
                            (rule_id,rule_version,record_id,field_name,field_value,test_result,error_message,timestamp,target_table,run_id)
                            VALUES(?,'1.0',?,?,?,?,?,?,'customers',?)""",
                            (
                                rule_id,
                                str(record["id"]),
